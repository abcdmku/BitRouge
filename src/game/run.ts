...
export const createRunSummary = (hub: HubState, run: RunState, cause: string, aborted: boolean): RunSummary => {
  ...
  return {
    ...
    // v2 banking: hub.data += run.dataMined + 5 × new max depths (no credit conversion)
    dataBanked: computeBankedData(run.dataMined + run.salvageData, newDepths),
    cause,
...
export const endRun = (state: GameState, cause: string, aborted = false): GameState => {
  ...
  const hub: HubState = {
    ...
    stats: {
      ...
      sitesCompleted: banked.stats.sitesCompleted + run.sitesCompleted,
      dataMined: banked.stats.dataMined + run.dataMined,
      payloadsDelivered: banked.stats.payloadsDelivered + run.payloadsDelivered,
      leaksCollected: banked.stats.leaksCollected + run.leaksCollected,
    },
